...
from app.services.profile_analyzer import search_person_info
...
class ProfessionalFeatures:
    ...
    # Search-based analysis (fallback when no social media)
    search_based: bool = False
    search_raw_analysis: str = ""
...
class FeatureExtractor:
    ...
    def search_person_by_name_and_job(
        self,
        full_name: str,
        employer: str = None,
        job_title: str = None,
        ...
        """
        Search for a person using their name, job, and other identifying info.

        This is a fallback when social media URLs aren't available or scraping fails.
        Uses Perplexity Sonar API to search the web for public information.
        ...
        Returns:
            Dict with search results and extracted features
        """
        # Build context string from available info
        context_parts = []
        if job_title:
            context_parts.append(job_title)
        if employer:
            context_parts.append(f"at {employer}")
        if location:
            context_parts.append(f"in {location}")

        additional_context = " ".join(context_parts) if context_parts else None

        # Search for professional info
    ...
    def extract_features_from_search(
        self,
        search_result: Dict[str, Any]
    ) -> ProfessionalFeatures:
        """
        Extract professional features from a name-based search result.

        Args:
            search_result: Result from search_person_by_name_and_job

        Returns:
            ProfessionalFeatures populated from search results
        """
        if search_result.get("status") != "success":
            return ProfessionalFeatures()

        raw_analysis = search_result.get("raw_analysis", "")

        if self.keywords_client:
            # Use AI to extract structured data
            return self.extract_professional_features(raw_analysis)
        else:
            # Manual extraction without AI client
            return self._extract_features_manually(raw_analysis)

    def _extract_features_manually(self, analysis_text: str) -> ProfessionalFeatures:
        """
        Extract features from text without AI client (rule-based fallback).
        """
        ...
        if "phd" in text_lower or "doctorate" in text_lower:
            features.education_level = 5
            features.has_advanced_degree = True
        ...
        # Industry detection
        for industry, stability in self.INDUSTRY_STABILITY.items():
            if industry in text_lower:
                features.industry = industry
                features.industry_stability = stability
                break

        # Career trajectory indicators
        if any(word in text_lower for word in ["promoted", "advanced", "growing", "leadership"]):
            features.career_trajectory = 4
        elif any(word in text_lower for word in ["stable", "consistent", "experienced"]):
            features.career_trajectory = 3

        # Experience years (look for patterns like "X years")
        import re
        years_match = re.search(r'(\d+)\+?\s*years?\s*(of\s*)?(experience|in the industry)', text_lower)
        if years_match:
            features.total_experience_years = float(years_match.group(1))

        # Job stability indicators
        if "long tenure" in text_lower or "long-term" in text_lower:
            features.job_stability_score = 0.8
        elif "frequent" in text_lower and ("change" in text_lower or "job" in text_lower):
            features.frequent_job_changes = True
            features.job_stability_score = 0.3

        # Red flag detection
        red_flags = ["gap", "unemployed", "laid off", "terminated", "inconsistent"]
        for flag in red_flags:
            if flag in text_lower:
                features.employment_gaps += 1
    ...
    def extract_all_features(
        ...
        applicant_employer: str = None,
        applicant_job_title: str = None,
        ...
        Args:
            ...
            applicant_name: Full name for fallback web search
            applicant_employer: Employer for fallback web search
            applicant_job_title: Job title for fallback web search
            applicant_location: Location for fallback web search
        ...
        if professional_analysis and self.keywords_client:
            ...
            # Fallback: search by name and job if no social media analysis
            print(f"No social media analysis - searching for {applicant_name}...")
            search_result = self.search_person_by_name_and_job(
            ...
            if search_result.get("status") == "success":
                professional = self.extract_features_from_search(search_result)
                # Store the raw search result for the report
                professional.search_based = True
                professional.search_raw_analysis = search_result.get("raw_analysis", "")
